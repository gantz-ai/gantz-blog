...
summary = "When agents execute code, things can go wrong - malicious prompts, buggy tools, unexpected commands. Limit the blast radius with sandboxing: containers for isolation, network policies that block unauthorized access, resource limits to prevent runaway processes, and syscall filtering with seccomp. Let agents run without risking your infrastructure."
draft = false
