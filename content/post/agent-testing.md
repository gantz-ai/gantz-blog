...
summary = "How do you test something that gives different answers each time? AI agents are non-deterministic by nature. Use unit tests for deterministic tool code, integration tests with mocked LLM responses for workflows, LLM-as-judge evaluation for output quality, and fuzzy matching for assertions. Run it all in CI. Testing agents is possible - just different."
draft = false
