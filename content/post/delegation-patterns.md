...
summary = "Not every agent should do everything. Build delegation systems where a coordinator routes tasks to specialists - code tasks go to the code agent, writing goes to the writer, research to the researcher. Learn capability registries that track what each agent can do, matching algorithms that pick the right agent, and load balancing across agent pools."
draft = false
